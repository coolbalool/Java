...
public class TicTacToeGUI implements ActionListener {
  ...
  public void checkWinner() {
    ...
    for (int i = 0; i < board.length; i++) {
      // row
      if (board[i][0].getText() == GameState.SPACE.text())
        continue;
    ...
    for (int i = 0; i < board.length; i++) {
      // col
      if (board[0][i].getText() == GameState.SPACE.text())
        continue;
    ...
    // (0,0) mid
    if (board[0][0].getText() == GameState.SPACE.text())
      return;
    ...
    // other mid
    if (board[0][board.length - 1].getText() == GameState.SPACE.text())
      return;
